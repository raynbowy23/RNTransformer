...
def valid_peds(epoch, val_loader, val_rn_dataset=None):
    ...
    for i, batch in pbar: 
        ...
        ### This arises error. There are only 32 peds but 62 peds are given. Maybe not aligned
        # Forward
        ...
        if opt.is_rn:
            ...
            rn_pred, traj_pred = model(V_obs_tmp, A_obs.squeeze(), ped_list=ped_list)
...
def main():
    ...
    if opt.optimizer == "Adam":
        ...
        optimizer = torch.optim.RMSprop(model.parameters(), lr=1.0, weight_decay=1e-3)
    elif opt.optimizer == "AdaGrad":
        optimizer = torch.optim.Adagrad(model.parameters(), weight_decay=5e-4)

    if opt.is_rn:
        optimizer_rn = torch.optim.RMSprop(model.model_rn.parameters(), lr=1e-4, weight_decay=1e-3)
