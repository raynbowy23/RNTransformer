...
class ConvTemporalGraphical(nn.Module):
    ...
    def __init__(self,
        ...
        self.x_w = nn.Parameter(torch.zeros(1), requires_grad=True)
...
class social_stgcnn(nn.Module):
    def __init__(self, n_stgcnn=1, n_txpcnn=1, input_feat=2, output_feat=5,
                 seq_len=8, pred_seq_len=12, kernel_size=3, num_nodes=None):
    ...
    def forward(self, v, a, h=None):

        # Encoder
        for k in range(self.n_stgcnn):
            v, a = self.st_gcns[k](v, a, h)
        ...
        v_mid = v.view(v.shape[0], v.shape[2], v.shape[1], v.shape[3])
